...
# to load and clean the data i have
def load_data():
    try:
...
if not data.empty:
    ...
    # [ST4] Sidebar with a label and image
    st.sidebar.header("Filter Options")
    st.sidebar.image("https://cdn-icons-png.flaticon.com/512/684/684908.png", use_container_width=True)
    ...
    # [VIZ2] Table for map detail
    # [ST3] DataFrame output/table
    # Table with detailed info
    st.write("Detailed Airport Info")
    st.dataframe(map_data[["name", "elevation_ft", "latitude", "longitude"]])

    # Shows the elevation in both feet and meters
    st.subheader("Elevation in Meters")
